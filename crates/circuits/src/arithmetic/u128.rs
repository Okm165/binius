...
use binius_field::{
	as_packed_field::PackScalar, packed::set_packed_slice, BinaryField128b, BinaryField1b, ExtensionField, Field, TowerField
};
...
where
	U: PackScalar<F> + PackScalar<BinaryField1b> + PackScalar<BinaryField128b> + Pod,
	F: TowerField + ExtensionField<BinaryField128b>,
{
	let packed = builder.add_packed(name, input, 7)?;
	if let Some(witness) = builder.witness() {
		witness.set(
			packed,
			witness
				.get::<BinaryField1b>(input)?
				.repacked::<BinaryField128b>(),
		)?;
	}
	Ok(packed)
}
	...
	F: TowerField,
{
	...
	let cin = builder.add_shifted("cin", cout, 1, 7, ShiftVariant::LogicalLeft)?;
	let zout = builder.add_committed("zout", log_rows, BinaryField1b::TOWER_LEVEL);

	if let Some(witness) = builder.witness() {
		(
			witness.get::<BinaryField1b>(xin)?.as_slice::<u128>(),
			witness.get::<BinaryField1b>(yin)?.as_slice::<u128>(),
			witness
			...
			.for_each(|(xin, yin, zout, cout, cin)| {
				...
				*cin = (*xin) ^ (*yin) ^ (*zout);
				*cout = ((carry as u128) << (u128::BITS - 1)) | (*cin >> 1);
			});
	}
	...
	F: TowerField,
{
	...
	let shifted = builder.add_shifted(name, input, offset, 7, ShiftVariant::LogicalLeft)?;
	if let Some(witness) = builder.witness() {
		(witness.new_column(shifted).as_mut_slice::<u128>(), witness.get(input)?.as_slice::<u128>())
			.into_par_iter()
	...
	F: TowerField,
{
	...
	let shifted = builder.add_shifted(name, input, offset, 7, ShiftVariant::LogicalRight)?;
	if let Some(witness) = builder.witness() {
		(witness.new_column(shifted).as_mut_slice::<u128>(), witness.get(input)?.as_slice::<u128>())
			.into_par_iter()
			.for_each(|(shifted, input)| *shifted = *input >> offset);
...
{
	let log_rows = builder.log_rows([input])?;
	anyhow::ensure!(log_rows >= 7, "Polynomial must have n_vars >= 7. Got {log_rows}");
	anyhow::ensure!(index < 32, "Only index values between 0 and 32 are allowed. Got {index}");

	let query = binius_core::polynomial::test_utils::decompose_index_to_hypercube_point(7, index);
	...
	if let Some(witness) = builder.witness() {
		...
		let input = witness.get(input)?.as_slice::<u128>();
	...
	F: TowerField + ExtensionField<BinaryField128b>,
{
	...
	let output = builder.add_committed("output", log_count + 7, BinaryField1b::TOWER_LEVEL);
	if let Some(witness) = builder.witness() {
		witness
	...
	let output_packed = builder.add_packed("output_packed", output, 7)?;
	let transparent = builder.add_transparent(
		"transparent",
		binius_core::transparent::constant::Constant::new(log_count, BinaryField128b::new(value)),
	)?;
	if let Some(witness) = builder.witness() {
		let packed = witness
...
mod tests {
	...
	fn test_mul_const() {
		...
		let a = builder.add_committed("a", 7, BinaryField1b::TOWER_LEVEL);
		if let Some(witness) = builder.witness() {
			witness
				.new_column::<BinaryField1b>(a)
				.as_mut_slice::<u128>()
				.iter_mut()
				.for_each(|v| *v = 0b01000000_00000000_00000000_00000000u128);
		}

		let _c = arithmetic::u128::mul_const(&mut builder, "mul3", a, 3, arithmetic::Flags::Checked)
			.unwrap();
